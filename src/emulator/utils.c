...
int barrel_shifter(ARM* proc, int shiftBy) {
  ...
  switch (shiftType) {
    ...
    case LOGICAL_LEFT:
      ...
      if (s) {
        int c = (shiftBy == 0) ? 0 : extract_bit((int*)&proc->registers[rm], WORD_SIZE*BITS_IN_BYTE - shiftBy);
  ...
  if (s) {
    int c = (shiftBy == 0) ? 0 : extract_bit((int*)&proc->registers[rm], shiftBy - 1);
    set_bit_to((int*)&proc->registers[CPSR_REGISTER], CPSR_C, c);
  }
...
int read_memory_bytes(ARM* proc, unsigned int addr) {
  ...
  int extracted_first = extract_bits((int*)&proc->memory[start_addr], start_bit, bits_remaining);

  // If unaligned, there are still bytes to read from the second row
  int extracted_second = extract_bits((int*)&proc->memory[start_addr+1], 0, start_bit);
...
void write_memory_bytes(ARM* proc, unsigned int data, unsigned int addr) {
  ...
  unsigned int lsb_data = extract_bits((int*)&data, 0, bits_remaining);
  unsigned int lower_memory = extract_bits((int*)&proc->memory[start_addr], 0, end_bit);

  // Shift the bytes which will be written on the second line
  unsigned int new_lower_mem = lower_memory | (lsb_data << end_bit);
  unsigned int higher_memory = extract_bits((int*)&proc->memory[start_addr+1], end_bit, bits_remaining);
  unsigned int msb_data = extract_bits((int*)&data, bits_remaining, end_bit);
